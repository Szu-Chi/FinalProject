...
color *Faded(BMP_Header Header, color* BMPcolor, int level) {
	color *data = calloc(Header.Height * Header.Width, sizeof(color));
	for (int i = 0; i < Header.Height; i++)
		for (unsigned int j = 0; j < Header.Width; j++) {
			int index = i*Header.Width + j;
			data[index].R = (BMPcolor[index].R<255 - level)?BMPcolor[index].R + level :255;
			data[index].G = (BMPcolor[index].G<255 - level)?BMPcolor[index].G + level :255;
			data[index].B = (BMPcolor[index].B<255 - level)?BMPcolor[index].B + level :255;
		}
	return data;
}

color *Reverse(BMP_Header Header, color* BMPColor) {
	color *data = calloc(Header.Height * Header.Width, sizeof(color));
	for (int i = 0; i < Header.Height; i++)
		for (unsigned int j = 0; j < Header.Width; j++) {
			int index = i*Header.Width + j;
			data[index].R = 255 - BMPColor[index].R;
			data[index].G = 255 - BMPColor[index].G;
			data[index].B = 255 - BMPColor[index].B;
		}
	return data;
}